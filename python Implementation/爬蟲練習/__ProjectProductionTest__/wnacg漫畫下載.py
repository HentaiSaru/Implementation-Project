...
# 初始化宣告
def __init__(self,FolderName,SaveName,Url,Image_URL,MangaURL,headers,pages_format,ComicLink):
...
class BasicDataProcessing:
    ...
    def BasicSettings(Url):
        ...
        if re.match(SupportedFormat,Url):
            ...
            #創建文件夾
            Create.Ffolder(NameMerge)
            # 開始請求圖片
            RetrieveData.DataRequest(total_pages_format,ComicLink,Url)
    ...
    def BatchInput(Url):
        ...
        if Judge:
            for _input in AllLinks: # 懶得處理線程鎖,廢棄多線程
                BasicDataProcessing.BasicSettings(_input)
    ...
    def DataRequest(pages_format,ComicLink,MangaURL):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=128) as executor:
            ...
            for page in pages_format:
                ...
                time.sleep(0.1)
                executor.submit(Create.Download, SaveName, MangaURL , Image_URL, headers)
...
"""
    ...
    Bug說明 : 因為網址格式是直接取得 , 該漫畫初始頁面的文字格式 , 因此只要該格式與圖片格式不符 , 就會請求失敗 (請求錯誤會顯示,錯誤的漫畫初始頁面網址,與錯誤的圖片網址)
...
if __name__ == "__main__":
    ...
    #BasicDataProcessing.BasicSettings("")
    ...
    BasicDataProcessing.BatchInput("")