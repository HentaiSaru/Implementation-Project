...
class DataProcessing:
    Save = queue.Queue() # 保存轉換後要輸出的數據
    Work = queue.Queue() # 保存要進行轉換的工作路徑
    Allow = ("txt", "ass") # 允許的檔案類型
...
class UICreation(DataProcessing):
    ...
    def Initial_UI(self):
        ...
        self.Content_items = tk.Text(
            self.Content_frame, font=("Arial Bold", 16),
            bg=buttonbackground, fg=buttontext,
    ...
    def __Conversion_output(self, index, work, OutType):
        ...
        with open(work, "r", encoding="utf-8") as text:
            with ThreadPoolExecutor(max_workers=100) as executor: # 多線程
                for txt in text: # 傳入數據回傳轉換
