...
echo:       ___________________________________________________
echo:   
call :_color2 %_White% "        " %_Green% "在鍵盤 [1,2,3,4,5] 中輸入菜單選項"
choice /C:12345 /N
set _erl=%errorlevel%
...
call :_color %Green% "IDM 激活 - 在註冊表中成功重置"
) else (
call :_color %Red% "無法完全重置 IDM 激活 - 試用"
)
...
if not [%errorlevel%]==[0] (
call :_color %Red% "無法連結 internetdownloadmanager.com，正在中止..."
goto done
)
...
echo %line%
echo:
call :_color %Green% "IDM 已成功激活"
echo:
call :_color %Gray% "如果盗版提示出现，再次運行激活選項，之后它就不會出現了"
goto done
)
