...
echo:
echo:          [1] 啟用 IDM
echo:          [2] 在登錄檔中重置 IDM 啟用/試用
echo:          _____________________________________________
echo:
call :_color2 %_White% "          [3] 切換 Windows 防火牆  " %_col% "[%_status%]"
echo:          _____________________________________________
echo:
echo:          [4] 說明文件
echo:          [5] 退出
echo:       ___________________________________________________
echo:
call :_color2 %_White% "        " %_Green% "在鍵盤 [1,2,3,4,5] 中輸入選單選項"
choice /C:12345 /N
set _erl=%errorlevel%
...
call :_color %Green% "IDM 啟用 - 在登錄檔中成功重置"
) else (
call :_color %Red% "無法完全重置 IDM 啟用 - 試用"
)
...
if not exist "!IDMan!" (
call :_color %Red% "未安裝 IDM [Internet Download Manager] "
echo 您可以從  https://www.internetdownloadmanager.com/download.html  下載
goto done
)
...
if not [%errorlevel%]==[0] (
call :_color %Red% "無法連線 internetdownloadmanager.com，正在中止..."
goto done
)
...
echo:
call :_color %Gray% "如果盜版提示出現，再次執行啟用選項，之後它就不會出現了"
goto done
)
